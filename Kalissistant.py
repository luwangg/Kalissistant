...
assistant = os.system("apt update && apt list --upgradable && apt -y install curl gnupg apt-transport-https  && curl https://packages.microsoft.com/keys/microsoft.asc | apt-key add - && echo 'deb [arch=amd64] https://packages.microsoft.com/repos/microsoft-debian-stretch-prod stretch main' > /etc/apt/sources.list.d/powershell.list && apt update && apt-get install -y python3-pip python3-crypto python-crypto")

# Clear Autoremove List
assistant += os.system("apt-get install -y libbabeltrace-ctf1 libcdio17 libfile-copy-recursive-perl libisl15 libllvm5.0 liblttng-ust-ctl4 liblttng-ust0 libtcl8.5 libtk8.5 libunwind8 python-subprocess32 python-unicodecsv python3-configargparse python3-flask python3-itsdangerous python3-pyinotify python3-simplejson python3-werkzeug tk8.5 libhttp-parser2.7.1 libsynctex1 openjdk-9-jdk-headless libedataserverui-1.2-1 libqgis-core2.18.17 libedataserver-1.2-22 libx265-146 libpoppler73 libcue1 openjdk-9-jdk libqgis-networkanalysis2.18.17 libappindicator1 openjdk-9-jre libcamel-1.2-60 libnfs8 libqgispython2.18.17 python3-jsbeautifier libqgis-gui2.18.17 && pip install osrframework crypto virtualenvwrapper scapy env pwn pybluez pwntools pycurl shodan netifaces python-nmap && pip3 install crypto && apt-get install -y veil veil-evasion && apt-get install -y powershell exiftool")

# Upgrade Kali
assistant += os.system("apt-get upgrade -y")

# Clear Autoremove List
assistant += os.system("apt-get install powershell")
